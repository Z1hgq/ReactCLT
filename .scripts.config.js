...
// We need compile additional content for react-clt user
function finalizeCompile() {
  if (fs.existsSync(path.join(__dirname, './lib'))) {
    // 为 dist/react-clt.less 编译入口文件
...
function buildThemeFile(theme, vars) {
  // Build less entry file: dist/react-clt.${theme}.less
  if (theme !== 'default') {
    fs.writeFileSync(
      path.join(process.cwd(), 'dist', `react-clt.${theme}.less`),
      `@import "../lib/style/${theme}.less";\n@import "../lib/style/components.less";`,
    );
    // eslint-disable-next-line no-console
    console.log(`Built a entry less file to dist/react-clt.${theme}.less`);
  } else {
    fs.writeFileSync(
...
function finalizeDist() {
  if (fs.existsSync(path.join(__dirname, './dist'))) {
    // Build less entry file: dist/react-clt.less
    fs.writeFileSync(
      path.join(process.cwd(), 'dist', 'react-clt.less'),
    ...
    // eslint-disable-next-line no-console
    console.log('Built a entry less file to dist/react-clt.less');
...
function getThemeVariables(options = {}) {
  let themeVar = {
    'hack': \`true;@import "\${require.resolve('react-clt/lib/style/color/colorPalette.less')}";\`,
    ...defaultTheme
  };
  if(options.dark) {
    themeVar = {
