...
const CloseIcon = props => {
  ...
  return (
    <span className="react-clt-mul-input-rect-suffix" onClick={onClick}>
      <i
...
const MulInput = forwardRef((props: MulInputProps, ref: SpanRef) => {
  ...
  return (
    <span
      ref={ref}
      className={cn('react-clt-mul-input', className)}
      style={style}
      onClick={() => {
    ...
    >
      <span onClick={e => e.stopPropagation()}>
        {tabs.map((item, index) => {
          return (
            <span key={index} className="react-clt-mul-input-rect">
              {item}
